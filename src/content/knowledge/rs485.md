...
The [IEG-100 Ethernet Industrial IoT Gateway](/products/ieg-100-ethernet-industrial-iot-gateway) is the IoTEdges product reference for Ethernet-based gateway deployments where RS485 Modbus data is collected from field devices.

## RS485 and Remote IO

Remote IO modules often use RS485 Modbus RTU so a gateway, RTU, or controller can read distributed DI/DO/AI/AO signals.

The [IEIO-100 Modbus Remote IO Module](/products/ieio-100-modbus-remote-io-module) is designed as a wired Modbus Remote IO family. Exact RS485 isolation, surge, ESD, register map, and electrical limits should remain unpublished during project engineering review.
...
The [IER-100 Ethernet Industrial RTU](/products/ier-100-ethernet-industrial-rtu) is the current IoTEdges product reference for local IO plus wired telemetry.

## Validation Notes

RS485 performance depends on wiring, device count, baud rate, noise environment, grounding, and firmware polling behavior. Keep exact point counts and distance claims confirmed during project engineering review during project engineering review.
