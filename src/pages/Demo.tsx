...
import { demoSiteCopy } from '../data/siteCopy';
    ...
    <div className="min-h-screen bg-slate-950 pt-8 pb-20 text-slate-300">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.22em] text-orange-400">{demoSiteCopy.eyebrow}</p>
            <h1 className="mb-2 text-3xl font-bold text-white" style={{ fontFamily: 'var(--font-display)' }}>
              {demoSiteCopy.heroTitle}
            </h1>
            <p className="max-w-2xl text-slate-400">
              {demoSiteCopy.heroDescription}
            </p>
          </div>
          <Link
              ...
              <div className="flex flex-col gap-3 border-b border-slate-200 bg-white px-4 py-4 sm:flex-row sm:items-center sm:justify-between lg:px-6">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h2 className="text-lg font-semibold text-slate-950">{demoSiteCopy.overviewLabel}</h2>
                    <span className="rounded-full bg-emerald-50 px-2.5 py-0.5 text-xs font-medium text-emerald-700 ring-1 ring-emerald-200">
                      {demoSiteCopy.overviewStatus}
                    </span>
                  </div>
                  <p className="mt-1 text-sm text-slate-500">{demoSiteCopy.overviewDescription}</p>
                </div>
                <div className="flex flex-wrap gap-2 text-xs">
                  {demoSiteCopy.overviewBadges.map((badge) => (
                    <span key={badge} className="rounded border border-slate-200 bg-slate-50 px-2.5 py-1 text-slate-600">{badge}</span>
                  ))}
                </div>
              </div>
          ...
          <div className="grid gap-12 md:grid-cols-2">
            <div>
              <h2 className="mb-6 text-2xl font-bold text-white" style={{ fontFamily: 'var(--font-display)' }}>
                {demoSiteCopy.bottomLeftTitle}
              </h2>
              {demoSiteCopy.bottomLeftParagraphs.map((paragraph, index) => (
                <p key={paragraph} className={`${index === 0 ? 'mb-6' : ''} leading-relaxed text-slate-400`}>
                  {paragraph}
                </p>
              ))}
            </div>
            ...
            <div>
              <h3 className="mb-6 text-xl font-bold text-white" style={{ fontFamily: 'var(--font-display)' }}>
                {demoSiteCopy.bottomRightTitle}
              </h3>
              <div className="space-y-5">
                {demoSiteCopy.bottomRightItems.map((item) => (
