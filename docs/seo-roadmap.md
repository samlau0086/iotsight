...
| Project accessories content | In progress | Added `/accessories` page covering recommended accessories for 4G antenna, SIM/APN, RS485 wiring, DIN rail power supplies, relay interfaces, sensors, meters and gate opener installations; added five accessory-focused Knowledge Base guides; added Compatible Accessories sections to all current hardware product pages. |
| CMS content model schema | Done | Added `docs/content-model-schema.md` covering structured schemas for products, solutions, blog posts, and knowledge pages so a future backend can preserve current layout, SEO prerendering, and section order. Frontmatter parsing baseline now uses `gray-matter` for nested structured fields. |
...
| Product structured frontmatter migration | Done | Moved `imageUrl`, `specGroups`, `selectionGuide`, `bomGroups`, and `preSalesFaq` into product Markdown frontmatter, and simplified `src/data/products.ts` to a pure Markdown loader. |
...
| Public product internal links | In progress | Home, gateway landing, solution detail architecture blocks, factory energy page, and first blog articles now link into public product pages. Legacy overclaims on public pages were reduced. Continue with solution and knowledge-base content expansion. |
...
| Solution Markdown pipeline | Done | Moved solution source of truth into `src/content/solutions/*.md` and converted `src/data/solutions.ts` into a loader, so solutions now follow the same content architecture as blog, knowledge, and products. |
...
10. Review GitHub Actions after the next push; the workflow now verifies prerendered product and knowledge pages before uploading the release to the VPS.
