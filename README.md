...
- Server-side live chat proxy integration with the ai-agent-live-chat deployment
...
flowchart LR
  ...
  Express --> AgentChat["ai-agent-live-chat API"]
  Express --> Stream["ai-agent-live-chat SSE stream"]
  AgentChat --> Desk["AI Agent / Human Agent Desk"]
```

The browser only calls this website's own endpoints:

- `GET /api/live-chat/widget-config`
- `PUT /api/live-chat/profile`
- `GET /api/live-chat/conversation`
- `POST /api/live-chat/messages`
- `GET /api/live-chat/stream`
- `POST /api/quote-request`

The server proxies those calls to `https://chat.iotedges.com/api/chat/*` by default.
...
Live route and API sample verification:

```bash
npm run verify:live-route-samples
```

This checks one live product route, one solution route, one knowledge route, one blog route, plus `/api/health` and `/api/quote-request`.
...
9. For a broader post-deploy check, run:
   - `npm run verify:production-surface`
   - `npm run verify:live-route-samples`
   - `npm run verify:cms-live-surface`
10. If a page should stay off the public site, keep its CMS `status` as `Draft` or `Review` instead of publishing it.
...
- Product, solution, blog, and knowledge pages now use controlled placeholder cover assets when an image field is empty at render time, but real project images or branded artwork should replace placeholders as soon as they are available.
- Related internal links are now partly derived from `relatedProducts`, `relatedResources`, and reverse content matches, so keeping those fields accurate improves both buyer navigation and SEO clustering.
- Public visibility rules are now explicit: Blog, Knowledge Base, and Solutions render publicly only when `status` is `Published`; Products remain public for `Published`, `Preview`, and `Available for project inquiry`, while `Draft` products are excluded from public routes and sitemap.

Post-publish checklist:

1. Confirm the GitHub Actions deployment workflow finished successfully.
...
- New images uploaded in CMS are written to `public/uploads`.
- Product pages rely on structured frontmatter. Do not casually rename `id`, `route`, `model`, or key spec fields after a page is live.
...
- `/admin/` shows the homepage:
  the live site is serving fallback HTML instead of the CMS admin shell.
- `/admin/config.yml` shows HTML:
  the VPS deployment or static route handling is stale.
- A product or solution page opens but shows homepage content:
  prerendered route output or static route serving is wrong on the live server.
- `Untitled Product` or similar placeholders appear:
  required frontmatter is missing from the Markdown source.
- `verify:live-route-samples` fails on `/api/quote-request`:
  check the live server build, Express API routes, and PM2 reload state.
...
- 通过服务端代理接入 ai-agent-live-chat 部署实例
...
服务端默认把这些请求代理到 `https://chat.iotedges.com/api/chat/*`。
...
这个命令会检查一个线上产品页、一个方案页、一个知识页、一个博客页，以及 `/api/health` 和 `/api/quote-request`。
...
| `LIVE_CHAT_API_BASE_URL` | 否 | ai-agent-live-chat 域名，默认 `https://chat.iotedges.com` |
...
9. 如果要做更完整的发布后抽查，执行：
   ...
   - `npm run verify:live-route-samples`
   - `npm run verify:cms-live-surface`
10. 如果内容还不该公开，不要把它设成 Published；保持 Draft 或 Review 即可。
...
- Product、Solution、Blog、Knowledge 页面现在都有受控 placeholder 封面回退策略，但一旦拿到真实产品图、项目图或品牌化封面，应该尽快替换掉 placeholder。
- 站内相关推荐现在会部分依据 `relatedProducts`、`relatedResources` 以及反向内容匹配自动生成，所以这些字段越准确，用户导航和 SEO 聚类效果就越好。
- 前台公开规则现在更明确：Blog、Knowledge Base、Solutions 只有 `status = Published` 才会进入公开页面、prerender 和 sitemap；Products 的 `Published`、`Preview`、`Available for project inquiry` 会保留公开，`Draft` 则不会出现在公开路由中。

发布后检查清单：

1. 确认 GitHub Actions 部署流程已经成功完成。
2. 打开线上页面，检查标题、主图、CTA 和相关链接是否正常。
3. 确认页面已经出现在 `https://iotedges.com/sitemap.xml` 中。
4. 检查 `https://iotedges.com/robots.txt` 仍然包含 sitemap 行和 `Disallow: /admin`。
5. 如果要做更完整的部署验收，运行 `npm run verify:cms-live-surface`。

常见内容操作说明：

- 在 CMS 中上传的新图片会写入 `public/uploads`。
- Product 页面依赖结构化 frontmatter，页面上线后不要随意修改 `id`、`route`、`model` 和关键规格字段。
- Solution 页面依赖 `link`、相关产品和架构图字段，这些值需要和真实路由保持一致。
- Blog 和 Knowledge 页面中的 `relatedProducts`、`relatedResources` 需要保持为真实存在的线上路由。
- Blog 的 Hero Image 现在视为常规必填封面图；Knowledge 页面如果没有单独封面，前端会自动使用受控知识库 placeholder。

常见故障检查：

- `/admin/` 打开后是首页：
  说明线上站点在返回 fallback HTML，而不是 CMS 后台壳。
- `/admin/config.yml` 打开后是 HTML：
  说明 VPS 部署版本或静态路由处理还是旧的。
- 产品页或方案页能打开，但内容看起来像首页：
  说明 prerender 输出或线上静态路由服务有问题。
- 出现 `Untitled Product` 等占位标题：
  说明 Markdown frontmatter 缺了必填字段。
- `verify:live-route-samples` 在 `/api/quote-request` 上失败：
  优先检查线上 server build、Express API 路由和 PM2 reload 状态。
