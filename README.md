...
IoTEdges is a React and Express powered website for industrial IoT solution marketing, product education, and website live chat lead capture.

The site presents IoTEdges solutions for factory energy monitoring, solar and renewable energy, water management, smart agriculture, and building automation. It also includes a live chat widget that connects website visitors to a TradeQuest CRM Live Chat Desk through a server-side proxy.
...
- Build-time prerendering for blog and solution pages to improve SEO
...
Adding a new `.md` file under `src/content/blog/` automatically adds it to the blog list after the next build or dev-server reload. The article URL is based on `id`, for example `id: my-post` becomes `/blog/my-post`. The list is sorted by `order` from low to high.

### SEO and Prerendering

`npm run build` runs a prerender step after the Vite client and SSR builds. The prerender script renders static HTML for:
...
For blog posts, the prerender step also writes article-specific `<title>`, meta description, and Open Graph tags based on Markdown frontmatter. This gives crawlers article HTML in the initial response instead of requiring them to execute the client bundle first.

Generated static files are written under `dist`, for example `dist/blog/my-post/index.html`. In production, `server.ts` checks for a prerendered `index.html` that matches the request path before falling back to the SPA `dist/index.html`.
...
- `VPS_DEPLOY_PATH`: optional deploy path, defaults to `/var/www/iotedges`
...
IoTEdges 是一个基于 React 和 Express 的工业物联网官网项目，用于展示解决方案、介绍产品能力，并通过网站 Live Chat 收集潜在客户线索。
...
- 构建时预渲染 Blog 和解决方案页面，提升 SEO 友好度
...
在 `src/content/blog/` 下新增 `.md` 文件后，下一次构建或开发服务刷新时会自动进入 Blog 列表。文章 URL 使用 frontmatter 里的 `id`，例如 `id: my-post` 会对应 `/blog/my-post`。列表会按 `order` 从小到大排序。

### SEO 和预渲染

`npm run build` 会在 Vite 客户端构建和 SSR 构建之后执行预渲染脚本。预渲染会为以下页面生成静态 HTML：
...
对于 Blog 文章，预渲染脚本还会根据 Markdown frontmatter 写入文章专属的 `<title>`、meta description 和 Open Graph 标签。这样爬虫在初始 HTML 响应中就能读取文章内容，而不必先执行前端 bundle。

生成的静态文件会写入 `dist`，例如 `dist/blog/my-post/index.html`。生产环境中，`server.ts` 会先查找与请求路径匹配的预渲染 `index.html`，找不到时再 fallback 到 SPA 的 `dist/index.html`。
...
- `VPS_DEPLOY_PATH`：可选，部署路径，默认 `/var/www/iotedges`
